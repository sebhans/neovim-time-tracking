...
vim.keymap.set({'i', 'n'}, '<C-s>',      '<Plug>(TimeTrackingQuit)',  {buffer = bufno})
vim.keymap.set({'i', 'n'}, '<C-CR>',     '<Plug>(TimeTrackingDone)',  {buffer = bufno})
vim.keymap.set({     'n'}, '<Leader>yr', '<Plug>(TimeTrackingClone)', {buffer = bufno, desc = "clone time tracking entry on current line"})
